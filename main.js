...
// html oluşturduk ve bunun içine kitapları gönderdik
...
const createBookTypesHtml = () => {
    ...
    let filterHtml = "";
    // fitre türlerini tutuacak dizi,"ALL" türüyle başlatılmıştır
    let filterTypes = ["ALL"];
...
const listBasketItems =() => {
  ...
  basketList.forEach((item) => {
    ...
    src="${item.product.imgSource}"
     ...
     <div class="basket__item-info">
      <h3 class="bool__name">${item.product.name}</h3>
      <span class="book__price">${item.product.price}tl</span> <br>
      <span class="book__remove" onClinck="removeItemToBasket(${item.product.id})">Sepetten Kaldır</span>
     </div>
  ...
  ? basketListHtml
  : `<li class="basket__item">Sepetete herhangi bir ürün bulunmuyor. Sepete ürün ekleyiniz.</li>`;
...
const addBookToBasket = (bookId) => {
  ...
  btnCheck.computedStyleMap.display = "block";
